...
const inputClass =
  "w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/30 text-sm text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500";
            ...
            <div className="flex flex-col items-center gap-3">
              <UploadButton
                ...
                onClientUploadComplete={async (res) => {
                  ...
                  if (!url) {
                    showStatus({ type: "error", msg: "Não foi possível obter a URL do arquivo." });
                    return;
                  }
                  ...
                  if (r.ok) {
                    showStatus({ type: "success", msg: `Documento "${res[0].name}" anexado com sucesso.` });
                    fetchCases();
                  } else {
                    showStatus({ type: "error", msg: "Arquivo enviado, mas falhou ao salvar no caso." });
                  }
                }}
                onUploadError={(err) => {
                  setUploading(false);
                  showStatus({ type: "error", msg: err.message });
                }}
              />
                        ...
                        return (
                          <li key={doc} className="flex items-center gap-1.5">
                            ...
                            <a
                              ...
                              className="text-xs text-sky-600 hover:underline truncate max-w-sm"
                            >
                              {name}
                            </a>
                          </li>
